    ...
    <div className="p-4 pb-24 space-y-4">
      ...
      <div className="ios-card flex items-center justify-between">
        ...
        <div className="text-center flex-1">
          <div className="text-ios-green text-lg font-bold">
            {tasks.filter((t) => (t.status ?? t.match_status) === 'accepted' || (t.status ?? t.match_status) === 'enroute').length}
          </div>
        ...
        <div className="text-center flex-1">
          <div className="text-ios-text text-lg font-bold">
            {tasks.filter((t) => (t.status ?? t.match_status) === 'completed').length}
          </div>
      ...
      <div className="space-y-3">
        {tasks.length === 0 ? (
          ...
          tasks.map((m) => {
            const ms = m.status ?? m.match_status ?? '';
            return (
